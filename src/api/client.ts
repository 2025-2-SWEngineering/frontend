...
export async function fetchTransactions(groupId: number, limit = 20, page = 1) {
  const { data } = await api.get<TransactionsListResponse>("/transactions", {
    params: { groupId, limit, page },
  });
  return data;
}

export async function fetchMonthly(groupId: number, months = 6) {
  const { data } = await api.get<MonthlyResponse>("/transactions/monthly", {
...
export async function setDues(groupId: number, userId: number, isPaid: boolean) {
  const { data } = await api.put<{ item: { user_id: number; is_paid: boolean; paid_at?: string } }>(
    "/dues",
    {
      groupId,
      userId,
...
export async function presignPut(filename: string, contentType: string) {
  const { data } = await api.post<PresignPutResponse>("/uploads/presign/put", {
    filename,
    contentType,
  });
  return data;
