...
export function useOverviewData(groupId: number | null) {
    ...
    const [stats, setStats] = React.useState<{ currentBalance: number; totalIncome: number; totalExpense: number } | null>(null);
    const [items, setItems] = React.useState<
        Array<{ id: number; type: "income" | "expense"; amount: number; description: string; date: string; receiptUrl?: string; category?: string; createdBy?: number }>
    >([]);
    const [monthly, setMonthly] = React.useState<Array<{ month: string; income: number; expense: number }>>([]);
    const [dues, setDues] = React.useState<Array<{ userId: number; userName: string; isPaid: boolean; paidAt?: string }>>([]);
    ...
    const loadInitial = React.useCallback(async () => {
        ...
        try {
            ...
            const m = ((monthlyData || []) as MonthlyResponse["data"]).map((r) => ({ month: r.month, income: Number(r.income), expense: Number(r.expense) }));
            setMonthly(m);
            const duesItems = await apiFetchDues(groupId);
            const mapped = ((duesItems || []) as DuesListResponse["items"]).map((r) => ({ userId: r.user_id, userName: r.user_name, isPaid: !!r.is_paid, paidAt: r.paid_at || undefined }));
            setDues(mapped);
            setTxPage(1);
        } finally {
            setLoading(false);
    ...
    const toggleDues = React.useCallback(async (userId: number, next: boolean) => {
        if (!groupId) return;
    ...
    return { loading, stats, items, monthly, dues, txHasMore, txMoreLoading, loadMore, refreshAll, toggleDues } as const;
}
